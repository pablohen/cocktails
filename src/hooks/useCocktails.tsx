...
export function useCocktails() {
  function Categories() {
    const fetchCategories = async () => {
      const res = await cocktailService.get<{ drinks: CategoryDTO[] }>("/list.php?c=list");
  ...
  function Drinks() {
    ...
    async function fetchDrinks(category: string) {
      const res = await cocktailService.get<{ drinks: DrinkDTO[] }>("/filter.php", {
        params: {
          c: category,
        },
      });
    ...
    async function fetchSearch(searchTerm: string) {
      const res = await cocktailService.get<{ drinks: DrinkDTO[] }>("/search.php", {
        params: {
          s: searchTerm,
        },
      });
  ...
  function Drink() {
    ...
    const fetchDrink = async (id: string) => {
      const res = await cocktailService.get<{ drinks: DrinkDTO[] }>("lookup.php", {
        params: {
          i: id,
        },
      });
